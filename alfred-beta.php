...
# this bot is just a script, like an irc bot. More features will be added :-)
# when starting the bot, use /add so the bot saves the default group name :-) 
...
#&& $chat_id == "CHAT_ID" IF U WANT TO ALLOW THE BOT IN JUST ONE CHAT(_ID)
    $title = $message['chat']['title'];
    $text = $message['text'];
    ...
    $titlefile = str_replace("-", "", $chat_id);

    $thereisatitle = (strlen(file_get_contents(".$titlefile.p8")) < 1 ? $thereisatitle = "send /add before anything" : $thereisatitle = file_get_contents(".$titlefile.p8"));
    #this will b changed in next update :-)
    $admin_list = array("ADD_ADMIN_USERNAMES_HERE");
    $admin_list_read = array("ADD_ADMIN_USERNAMES_HERE_WITH_@");

    switch ($comando) {
      case '/add':
        if(in_array($username, $admin_list)) {
          $savetitle = fopen(".$titlefile.p8", 'w+');
          fwrite($savetitle, $title);
          fclose($savetitle);
          apiRequest("sendMessage", array('chat_id' => $chat_id, 'reply_to_message_id' => $message_id, 'text' => "Nome salvo"));
        }
        break;

      case '/ginfo':
        $ginfo = "Title: $thereisatitle\nID: $chat_id";
      ...
      case '/promote':
        ...
        if(in_array($username, $admin_list)){
          ...
          if($promote)
            apiRequest("sendMessage", array('chat_id' => $chat_id, 'reply_to_message_id' => $message_id, 'text' => "Usuário $iusername agora é moderador"));
        }
      ...
      case '/demote':
        ...
        if(in_array($username, $admin_list)){
          ...
          if($demote)
            apiRequest("sendMessage", array('chat_id' => $chat_id, 'reply_to_message_id' => $message_id, 'text' => "Usuário $iusername agora não é moderador"));
        }
      ...
      case '/settopic':
        ...
        if($split[1] == "-d") {
          $setnamed = apiRequest("setChatTitle", array('chat_id' => $chat_id, 'title' => "$thereisatitle"));
          if ($setnamed) {
            $logtopic = "#new_topic #default_name\nGrupo: $title\nTopic: $thereisatitle\n\n#by_user\nUsername: @$username\nId: #id$userid";
            apiRequest("sendMessage", array('chat_id' => $chat_id, 'reply_to_message_id' => $message_id, 'text' => "Default name."));
            apiRequest("sendMessage", array('chat_id' => "LOG_CHANNEL_ID", 'text' => "$logtopic"));
          }
        } else {
          if(in_array($username, $admin_list) && strlen($topico) > 2){
            $logtopic = "#new_topic\nGrupo: $title\nTopic: $topico\n\n#by_user\nUsername: @$username\nId: #id$userid";
            $setname = apiRequest("setChatTitle", array('chat_id' => $chat_id, 'title' => "$thereisatitle | $topico"));
            if($setname) {
